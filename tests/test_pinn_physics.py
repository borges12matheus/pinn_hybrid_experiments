...
PINN_MOM_CONFIG = ROOT / "configs" / "physics_cont_mom" / "pinn_mom_v1.yaml"
...
class PinnPhysicsTest(unittest.TestCase):
    ...
    def test_momentum_config_is_defined(self):
        config = load_yaml(PINN_MOM_CONFIG)

        self.assertEqual(config["pinn"]["physics_mode"], "momentum")
        self.assertEqual(config["pinn"]["nut_transform"], "exp")
        self.assertIn("w_mom", config["pinn"])
        self.assertGreater(config["pinn"]["epochs_pre"], 0)
        self.assertGreater(config["pinn"]["epochs_phys"], 0)
    ...
    def test_shared_experiment_contract_isometric(self):
        ...
        pinn_mom = load_yaml(PINN_MOM_CONFIG)

        # "features" varia entre variantes PINN por design: cont_mom_v1 e
        # mom_v1 exigem entradas físicas extras (k, nut_log, Re) para o
        # resíduo de PDE, que a MLP baseline não usa.
        shared_sections = [
            "paths",
            "targets",
            "training",
